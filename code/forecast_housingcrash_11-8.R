...
#setwd("D:/Studies/Teaching/Classes/Econ 485/2022/R")
...
house <- ts_cansim("v111955442", start = date.start)
...
data <- cbind(INF, GDP, HOUSE, TARGET, GDP.US, WTI)
plot(data)
...
causality(mod.restrict, cause = 'HOUSE')
...
house.path <- c(126, 125, 124, 123, 121, 120, 119.7, 119, 118, 116, 115, 114, 113, 112.644)
house.path <- diff(house.path)

length(diff(house.path))

window(data.fc[, "HOUSE"], start = c(2023, 01), end = c(2023, 12)) <-
  c(diff(house.path))
