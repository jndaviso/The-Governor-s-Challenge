...
cpi.median <- ts_cansim("v108785714", start = date.start)
cpi.trim <- ts_cansim("v108785715", start = date.start)
...
target <- ts(target$VALUE, start = c(1994, 01), frequency = 12)
...
##################################################################################################### Rotate Trim and Median in for INF
...
# data <- cbind(INF, GDP, TARGET, GDP.US, WTI)
# data <- cbind(INF, GDP, GDP.US)
# data <- cbind(INF, GDP, U, TARGET, GDP.US, WTI)
# data <- cbind(INF, GDP, U, TARGET, GDP.US)
data <- cbind(INF, GDP, TARGET, GDP.US)
plot(data)
...
mat.coef.res
mat.coef.res['GDP.US', 1:3] <- 0
mat.coef.res['GDP.US', 5:7] <- 0
mat.coef.res['GDP.US', 9:11] <- 0
#mat.coef.res[c('WTI','GDP.US'), 1:3] <- 0     # 1. Lag
#mat.coef.res[c('WTI','GDP.US'), 6:8] <- 0    # 2. Lag
#mat.coef.res[c('WTI','GDP.US'), 11:13] <- 0   # 3. Lag
mat.coef.res
...
window(data.fc[, "TARGET"], start = c(2022, 12), end = c(2023, 7)) <-
  c(rep(4, 2), rep(3.75, 2), rep(3.5, 2), rep(3.25, 2))
...
DATE <- as.Date(time(INF.fc))
class(DATE)

#df2 <- data.frame(cbind(INF = INF.fc, GDP = GDP.fc, U = U.fc,
#                        TARGET = TARGET.fc))
df2 <- data.frame(U = U.fc)




write.xlsx(df2, "output/forecast_unemp.xlsx")
